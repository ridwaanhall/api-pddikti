...
@router.get("/search/all/{keyword}/", summary="Search all entities")
def search_all(keyword: str):
    return as_json(api_client.get_with_keyword("pencarian/all", keyword))


@router.get("/search/pt/{keyword}/", summary="Search universities")
def search_pt(keyword: str):
    return as_json(api_client.get_with_keyword("pencarian/pt", keyword))


@router.get("/search/prodi/{keyword}/", summary="Search study programs")
def search_prodi(keyword: str):
    return as_json(api_client.get_with_keyword("pencarian/prodi", keyword))


@router.get("/search/dosen/{keyword}/", summary="Search lecturers")
def search_dosen(keyword: str):
    return as_json(api_client.get_with_keyword("pencarian/dosen", keyword))


@router.get("/search/mhs/{keyword}/", summary="Search students")
